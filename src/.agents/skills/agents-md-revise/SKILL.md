...
## Project-specific overrides for gx

When revising `AGENTS.md` during a gx session, pay attention to:

- **New git functions** added to `internal/git/` — the AGENTS.md has a "Git domain patterns" section documenting each function's signature and behavior. Add new entries there.
- **Architecture layer table** — if a new package is created, update the table at the top ("Strict layered architecture").
- **Common agent mistakes** section at the bottom — append new gotchas discovered during this session.
- **Test scripts** — if a new test file is added, note it in the "Testing" section.
- **New commands** — the AGENTS.md has a "Currently implemented" section listing all commands and their status.
